...
# 404エラーを避けるため、検索ツールを一旦無効化して標準モデルを起動
try:
    ...
    if st.button("AIに相談する"):
        if not (sub_title or hypothesis):
            ...
            with st.spinner("AI指導員がプランを精査中..."):
                prompt = f"""
                あなたは材料科学の専門家です。提供された断片的な実験計画に対し、物理化学の原理に基づき厳密に査読し、不足部分を補完してアドバイスしてください。
                ...
                2. 具体的数値の逆提案: 「未入力」の項目がある場合や、設定が不十分な場合、専門家の視点で具体的な数値（温度、濃度等）を提案してください。
                3. 推奨される参考文献・キーワード: この検証の裏付けとなる著名な研究や、検索すべき英語キーワードを提示してください。
                4. 次のアクション: 予備実験で優先的に確認すべき事項。
                """
                try:
...
if st.button("PDFレポートを生成・保存"):
    ...
    if os.path.exists(font_path):
        ...
        st.error("フォントファイル(ipaexg.ttf)が見つかりません。")
